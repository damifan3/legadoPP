 ...
 * 定时任务日志对话框
 *
 * 文件位置：legado/app/src/main/java/io/legado/app/ui/autoTask/AutoTaskLogDialog.kt
 * 作用：显示定时任务的执行日志。
 *
 * 主要功能：
 * - 显示上次执行时间和日志内容
...
class AutoTaskLogDialog() : BaseDialogFragment(R.layout.dialog_recycler_view),
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val message = task?.lastLog ?: task?.lastError ?: getString(R.string.auto_task_not_run)
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        if (item?.itemId == R.id.menu_clear) {
            ...
            AutoTask.update(taskId) {
                it.copy(lastLog = null, lastError = null)
            }
            adapter.setItems(
                listOf(
                    Triple(
